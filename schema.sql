...
CREATE TABLE IF NOT EXISTS public.products (
    id TEXT PRIMARY KEY, -- Slug-based ID for SEO and URL stability
    name TEXT NOT NULL,
    urdu_name TEXT,
    description TEXT,
...
-- 6. Role-based Access for Products
-- Assuming we'll use a 'role' in raw_app_meta_data or a separate table
-- For a robust setup, we refine the "Allow authenticated management"
DROP POLICY "Allow authenticated management" ON public.products;
CREATE POLICY "Allow admin management" ON public.products
    FOR ALL USING (
        auth.role() = 'authenticated' AND 
        (auth.jwt() -> 'app_metadata' ->> 'role') = 'admin'
    );

-- 7. Carts Table (JSONB storage for simplicity)
